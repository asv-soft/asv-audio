...
public class OpusEncoder : DisposableOnceWithCancel, IObservable<ReadOnlyMemory<byte>>
{
    ...
    )
    {
        ...
        _encoder = OpusNative.OpusEncoderCreate(
            pcmFormat.SampleRate,
            pcmFormat.Channel,
            (int)settings.Application,
            out var error
        );
        ...
        CheckError(
            OpusNative.OpusEncoderCtl(
                _encoder,
                OpusCtl.OpusSetInbandFecRequest,
                settings.ForwardErrorCorrection ? 1 : 0
            )
        );
        CheckError(
            OpusNative.OpusEncoderCtl(
                _encoder,
                OpusCtl.OpusSetBitrateRequest,
                settings.CodecBitrate
            )
        );
        CheckError(
            OpusNative.OpusEncoderCtl(_encoder, OpusCtl.OpusSetSignalRequest, (int)settings.Signal)
        );
        CheckError(
            OpusNative.OpusEncoderCtl(
                _encoder,
                OpusCtl.OpusSetBandwidthRequest,
                (int)settings.Bandwidth
            )
        );
        CheckError(
            OpusNative.OpusEncoderCtl(
                _encoder,
                OpusCtl.OpusSetComplexityRequest,
                settings.Complexity
            )
        );
        CheckError(
            OpusNative.OpusEncoderCtl(
                _encoder,
                OpusCtl.OpusSetForceChannelsRequest,
                (int)settings.ForceChannels
            )
        );
        CheckError(
            OpusNative.OpusEncoderCtl(
                _encoder,
                OpusCtl.OpusSetInbandFecRequest,
                (int)settings.InbandFecMode
            )
        );
        CheckError(
            OpusNative.OpusEncoderCtl(_encoder, OpusCtl.OpusSetDtxRequest, (int)settings.DtxMode)
        );
        CheckError(
            OpusNative.OpusEncoderCtl(
                _encoder,
                OpusCtl.OpusSetPredictionDisabledRequest,
