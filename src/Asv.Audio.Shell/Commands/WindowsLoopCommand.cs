...
internal class WindowsLoopCommand : Command<WindowsLoopCommand.Settings>
{
    ...
    public WindowsLoopCommand()
    {
        ...
        using var rec =
            src.CreateFirstCaptureDevice(format) ?? throw new Exception("Capture device not found");
        using var play =
            src.CreateFirstRenderDevice(format) ?? throw new Exception("Render device not found");
        ...
        rec.OpusEncode(format)
            .OpusDecode(format) // may be some processing here (e.g. send over network)
            .Subscribe(play);
    }
    ...
    public override int Execute(CommandContext context, Settings settings)
    {
        ...
        if (renderDevice is not null && captureDevice is not null)
        {
