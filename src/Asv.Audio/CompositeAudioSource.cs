...
public class CompositeAudioSource:IAudioSource
{
    ...
    public CompositeAudioSource(string id, params IAudioSource[] sources)
    {
        ...
        _sources = sources;
        Id = id;
        CaptureDevices = sources.Select(x => x.CaptureDevices).Merge().Transform(x => 
            (IAudioDeviceInfo)new CompositeDeviceInfo(x));
        RenderDevices = sources.Select(x => x.RenderDevices).Merge().Transform(x => 
            (IAudioDeviceInfo)new CompositeDeviceInfo(x));
    }
...
public class CompositeDeviceInfo(IAudioDeviceInfo audioDeviceInfo) : IAudioDeviceInfo
{
    ...
    public string Id { get; } = string.Join(CompositeAudioSource.Delimiter, audioDeviceInfo.Source.Id, audioDeviceInfo.Id);
